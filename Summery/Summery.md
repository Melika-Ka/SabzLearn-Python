...
## S07-E11-E12-string-methods-part1-part2

- `string.casefold()` => تبدیل حروف انگلیسی بزرگ رشته به حروف کوچک
- `string.lower()` => تبدیل حروف انگلیسی بزرگ رشته به حروف کوچک
    - متد casefold از قدرت و نفوذ بیشتری نسبت به lower برخوردار است.
- `string.center(number, "sign")` => برای مرتب کردن یک متن استفاده میشود
- `repr(string)` => رشته را داخل کتیشن قرار میدهد
- `string.expandtabs(number)` => تب های رشته را با فاصله جایگزین میکند، میتوانیم مشخص کنیم چند فاصله
  بگذارد
- `string.format_map(dict_name)` => میتوانیم به مقدار کلیدهای دیکشنری در رشتهمان دست یابیم
- `string.index("char")` => ایندکس یک کاراکتر را در رشته مشخص میکند
- `string.rindex("char")` => برای جستجو از سمت راست رشته از rindex استفاده میکنیم
- `string.find("char")` => ایندکس یک کاراکتر را در رشته مشخص میکند
    - گر متد index یک کاراکتر را پیدا نکند خطا میدهد اما find خطا نمیدهد بلکه -1 را نشان میدهد.
- `string.isalpha()` => برای مشخص کردن اینکه همه کاراکترهای رشته حروف الفبا هستند یا خیر
- `string.isascii()` => برای مشخص کردن اینکه همه کاراکترهای رشته اسکی هستند یا خیر
- `string.isdecimal()` `<=` `string.isnumeric()` `<=` `string.isdigit()` => برای مشخص کردن اینکه همه
  کاراکترهای رشته عدد هستند یا خیر
- `string.isidentifier()` => برای مشخص کردن اینکه اسم یک متغیر قابل استفاده هست یا خیر
- `string.islower()`=> برای مشخص کردن اینکه آیا همهی کاراکترهای رشته حرف کوچک هستند یا خیر
- `string.isprintable()` => برای مشخص کردن اینکه آیا همهی کاراکترهای رشته قابل چاپ هستند یا خیر
- `string.isspace()` => برای مشخص کردن اینکه آیا همهی کاراکترهای رشته فاصله هستند یا خیر از متد
- `string.title()` => حرف اول همهی کلمات رشته را به حرف بزرگ تبدیل میکند و بقیهی حرفها را به حرف
  کوچک
- `string.istitle()` => برای مشخص کردن اینکه حرف اول همهی کلمات داخل رشته حرف بزرگ و بقیهی حرفها
  حرف کوچک هستند یا خیر
- `string.isupper()` => برای مشخص کردن اینکه همهی حروف رشته، حرف بزرگ هستند یا خیر
- `string.ljust(number)`=>برای مرتب کردن یک متن استفاده میشود
    - `string.ljust(number ,"char")`
- `string.rjust(number, "char")` => رشته را در سمت راست و فاصله و یا کاراکتری که تعیین کردیم را قبل
  از رشته قرار میدهد
- `string.maketrancs("char1" , "char2" , "char3")` => کاراکترهای مورد نظرمان را نشان میدهد و
  کاراکتر دوم را با کاراکتر اول جابجا میکند و کاراکتر سوم را حذف میکند
- `string.translate(maketrans_name)` => برای اعمال این تغییرات از متد translate استفاده میکنیم و به
  عنوان ورودی به آن maketrans را میدهیم.
    - در متد maketrans می توانیم به عنوان ورودی دیکشنری بدهیم.
- `string.partition("char")` => یک رشته را با توجه به رشتهای که به این متد میدهیم به سه قسمت قبل و
  خود این رشته و بعد آن تقسیم میکند
- `string.rpartition("char")` => اگر بخواهیم از آخر دنبال رشتهی مورد نظرمان بگردد
- `string.removeprefix("char")` => از اول رشته، رشته ی مورد نظر ما را حذف میکند
- `string.removesuffix()` => از آخر رشته، رشتهی مورد نظر ما را حذف میکند
    - تفاوت متدهای removeprefix و removesuffix با متدهای lstrip و rstrip این است که برای متدهای
      lstrip
      و rstrip تعداد مهم نیست.
- `string.split("char" , number)` => یک لیست میسازد که عناصرش را بر اساس ورودی که میدهیم جدا میکند
- `string.rsplit("char" , number)` => جداسازی را از سمت راست انجام میدهد
- `string.splitlines()` => یک لیست میسازد که عناصر را براساس n \جدا میکند
- `string.splitlines(True)` => اگر مقدار True بنویسیم n \را هم نشان میدهد
- `string.swapcase()` => تمام حروف کوچک را به حروف بزرگ و تمام حروف بزرگ را به حروف کوچک تبدیل میکند
- `string.zfill(number)` => به تعداد دلخواه قبل از رشته 0 گذاشت

# S08

## S08-E01-modular-programming

- modular-programming : به تقسیم یک پروژهی بزرگ به تسکها و ماژولهای کوچکتر جهت مدیریت بهتر، برنامه
  نویسی ماژوالر میگویند.
- benefits :
    - اشکالزدایی آسانتر
